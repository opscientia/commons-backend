...
const datasetSchema = yup?.object().shape({
  ...
  chunkIds: yup
    .array()
    .of(
      yup.mixed().__inputType((input: any) => input instanceof mongodb.ObjectId)
    ), // chunkIds
});
...
const chunkSchema = yup?.object().shape({
  datasetId: yup
    .mixed()
    .__inputType((input: any) => input instanceof mongodb.ObjectId)
    .required(),
  path: yup.string(),
  doi: yup.string(),
  storageIds: yup
...
export async function validateDataset(dataset: Dataset): Promise<boolean> {
  if (!dataset) throw console.error("dataset undefined");
...
export async function validateCommonsFile(
